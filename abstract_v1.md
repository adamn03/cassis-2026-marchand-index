...
The figure presents the 14 players ranked **two ways side by side**: by `engagement_raw` (raw popularity) and by Marchand Index (cap-adjusted, market-controlled). **The divergence between the two rankings is the methodological point** — it shows that cap- and market-adjusted attention is not merely a re-skin of popularity.

Pre-registered expected patterns (reported regardless of outcome): (i) the cap-adjusted ranking surfaces Marchand and Reaves above raw-popularity leaders; (ii) Marner's `OAQ_observed`–`OAQ_portable` gap is materially larger than Reaves' (market amplification visible vs. portable attention); (iii) at least two top-5 rank flips between the two columns. **Patterns are illustrative, not pass/fail gates** — failure to observe a pattern is reported as a sensitivity finding, not as a failure of the abstract.

A representative worked example on the figure shows Marchand's K=5 peer subset, his `OAQ_observed` and `OAQ_portable` with 95% bootstrap CIs, and the resulting Marchand Index. Full N=~700, K=10 leaguewide matching is the Wk 6 deliverable of the build plan.
