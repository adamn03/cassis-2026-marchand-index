...
| 3 | Pre-registered hypotheses | Locked in `docs/preregistration.md` BEFORE the production model runs |
...
## Candidate ideas (NOT active projects — only ONE will be built)

**Both files in this folder are IDEAS at the spec stage. No code has been written for either. We are choosing between them and will build only ONE for CASSIS.** The May 31, 2026 abstract deadline forces this selection soon.

| File | What | Status |
|---|---|---|
| `NHL_Draft_Model.md` | On-ice prospect evaluation idea | Spec-stage — competing for selection |
| `NHL_Marchand_Index.md` | Off-ice fan-attention model idea | Spec-stage — competing for selection |

**Selection status:** OPEN. Lock this decision with the owner before doing any build work.

When asked to work on "the project," ask which one is selected unless context makes it obvious. Do not assume. Do not start building one while the other is still in the running.

Once selected, the chosen project will use: vault Python defaults, atomic-write convention, OpenRouter LLM access, and any scraping infrastructure it spins up (EliteProspects mirror, etc.) is built from scratch as part of that build.
...
- H1–H4 pre-registered in `docs/preregistration.md` BEFORE the model runs on production data.
...
- Plans: full project specs in this folder (`NHL_Draft_Model.md`, `NHL_Marchand_Index.md`)
