...
### A5 — 2026-05-22: Full-build Gate 4 (stratified generalization) added to validation plan

**Section affected:** §12 (out-of-scope list) — clarification only; no change to locked pilot method, weights, peer matching, or expected-pattern hypotheses P1–P3.
**Change:** A fourth full-build validation gate — **Stratified Generalization / Non-Star Signal Validation** — has been added to the project. It tests whether `OAQ_portable` predicts independent, held-out attention residuals (YouTube view counts under a controlled regression) outside the star tier, with players pre-sorted into star / regular / depth bands using non-OAQ variables (cap percentile, TOI/G percentile by position, points/60 percentile by position, All-Star history, jersey-list history). The full Gate 4 specification — band rules, sampling frame, snapshot rule, deduplication, coverage floor, null-result triggers, bootstrap procedure — lives in `docs/preregistration.md`, locked on the same date as this amendment.
**Why this amendment is logged here despite being a full-build addition:** §12 of this pilot pre-reg lists "Three-validation gates (jersey list ρ, All-Star vote ρ, FA-signing event study — Wk 7–8)" as explicitly out-of-scope for the pilot. That sentence remains literally true (Gate 4 is also out-of-scope for the pilot), but the validation plan referenced by the abstract has changed from three gates to four. This note is the audit trail. The pilot itself — its 14-player set, K=5 peer pool, composite formula, P1/P2/P3 expected patterns, and §11 fallback-to-schematic rule — is unchanged.
**No re-run of the pilot.** The pilot results in `pilot/oaq_pilot.csv`, `pilot/results.md`, and `pilot/figure.png` are not re-computed. Gate 4 belongs to the full-build phase (post-submission, Wk 7–8 of `NHL_Marchand_Index.md`).

### A6 — 2026-05-24: §4 figure scope amended — confirmed P3 top-5 with real player names, replacing synthetic schematic

**Section affected:** §11 (fallback-to-schematic rule).
**Change:** The §4 figure is now a real-data visualization of the confirmed P3 rank reordering (top 5 by `engagement_raw` vs. top 5 by `marchand_index`) using real player names and real cap hits from the pilot output, rather than the synthetic-data schematic originally specified by §11.
**Why this departs from §11:** §11 specified a synthetic schematic when ≥2 pre-registered patterns are disconfirmed, to prevent presenting tainted ranks as if they were the validated method's output. The new figure restricts the visualization to the **confirmed P3 portion only** — the top 5 of each ranking — which is the part of the pilot output that was not driven by the disconfirmed market-baseline proxy. The proxy-tainted lower ranks (e.g., Reaves' Marchand Index of −2.43, Marner's smaller-than-expected `|OAQ_observed − OAQ_portable|` gap) and the P1 / P2 disconfirmations themselves are not visualized; they are described in §2 of the CASSIS abstract and remain in `pilot/oaq_pilot.csv`. The amendment therefore preserves §11's spirit (no visualization of tainted ranks) while honouring the project owner's request to make the §4 figure concrete enough for reviewers to verify the rank-flip claim against named NHL players.
**Implementation.** Figure regenerated by `pilot/render_figure_v2.py`. `pilot/render_figure.py` (the original schematic renderer) is preserved in the repository for audit but is no longer the source of `pilot/figure.png`.
**No re-run of the pilot data.** Only the figure was regenerated. `pilot/oaq_pilot.csv`, `pilot/results.md`, and `pilot/results.json` are unchanged.
