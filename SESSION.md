...
LAST: Exploratory in-memory index runs (no files, sacred CSVs untouched) — eyeballed third-liners / top-D / top-overall by OAQ_portable. Two REVIEW-PENDING items surfaced + logged below: (#1) a skill-input DATA BUG — found, fixed, refetched, verified; (#2) an OPEN methodology question on OAQ_portable low-tail behavior — neutral dossier + Tests 1-4, decision left to stronger model. `fetch_nhl_api.py` patched + `raw/nhl_skill.csv` regenerated; BOTH COMMITTED in `a216941`.
...
→ COMMITTED in `a216941` (both `fetch_nhl_api.py` and the regenerated `raw/nhl_skill.csv`). No open correctness question remains.
...
## OWNER DATA REQUEST (added 2026-07-29) — ASK FOR THIS WHEN OWNER SAYS "CONTINUE"
Owner will hand-collect these. **Ask for them at the start of the next session.** Do not attempt to scrape.

**What to ask for — 32 rows:** `team_code, ig_handle, ig_followers, x_handle, x_followers, read_date`
(one read_date for the whole batch is fine).

**Why (ties directly to OPEN ITEM #3B above):** `market_z`'s social component is `team_sub_subscribers` — Reddit only. Reddit is English/US-skewed, so it mis-measures fanbase size per market, which is exactly the under-correction #3B point 10 asks about. Measured 2026-07-29 against a published Jun-2024 IG list:

| Team | metro pop | Reddit subs | IG followers | IG/Reddit |
|--|--|--|--|--|
| PIT | 2.42M | 118,110 | 2,700,000 | 22.9x |
| MON | 4.29M | 101,589 | 923,000 | 9.1x |
| WPG | 0.83M | 77,402 | 453,000 | 5.9x |
| TOR | 6.20M | 359,680 | 1,330,000 | 3.7x |
| VAN | 2.64M | 189,560 | 580,000 | 3.1x |

Platform ratio spans **7x**. Reddit ranks VAN above MON by 1.87x; IG ranks MON above VAN by 1.59x — the platforms disagree on DIRECTION, not just magnitude. MON is the 2nd-largest metro but only 6th in Reddit subs (r/Habs is an English proxy for a francophone market). Wayback X counts (2020-2022) put MON near the top too — a third platform agreeing with IG against Reddit. Plausible mechanism for #3B: MON social z depressed -> `market_z` depressed -> `λ·max(0,market_z)` under-subtracts -> MON `OAQ_portable` inflated -> 16/26 Habs in top-100.

**Sourcing constraints (all verified 2026-07-29, do not re-litigate):**
- IG + X unauthenticated = JS shell, no count in HTML (IG returns 605KB, zero `og:description`). Free aggregators dead: instastatistics 429, socialblade 404. Manual read is the only $0 path — matches how `metro_population` / `arena_attendance` are already hardcoded + cited in `fetch_market_proxy.py:89-205` + `market_proxy_sources.md`.
- Reddit subs are FROZEN at 2025-02-14 (18 teams) / 2025-02-15 (13) / UTA split. Arctic Shift serves one crawl per sub, no date parameter; `reddit.com/r/*/about.json` still 403 at $0. Cannot refresh, cannot backdate.
- => mixed vintages across the 3 platforms are unavoidable. Acceptable in principle because `market_z` is a z-score/ranking across 32 teams, not a windowed flow — and the other components are already off-window (StatCan 2021 census, 2025 US est, season-avg attendance). Drift spot-check on 4 teams via Wayback X (2020-06 -> 2021-11, 17 months): growth spread 1.06x, one adjacent swap (MTL/BOS, 1.5% apart). n=4 — widen to 32 if this becomes load-bearing.
- NHLFollowTrack (@NHLFollowTrack) is NOT usable: it tracks who players FOLLOW/UNFOLLOW on Instagram, not follower counts. $2.99/mo app, no API, no public data. Dead end — do not revisit.

**HOW to incorporate: DELIBERATELY UNDECIDED.** Owner deferred the design. Nothing pre-committed, no amendment written, A45 still free. Note only: A12 removed follower counts as a PLAYER-LEVEL CES component on stock-vs-flow grounds; that objection does not automatically transfer to a TEAM-LEVEL market-size control (metro pop + arena capacity are already stocks). A30 dropped `team_social_followers` for ACCESS (403), not concept.

NEXT: #1 skill fix is CORRECTNESS-VERIFIED pool-wide (764/764 == NHL published totals) and COMMITTED (`a216941`) — CLOSED; stronger-model pass on it is optional/confirmatory only. Genuinely-open items for the stronger model are #2 (OAQ_portable low-tail behavior, Tests 1-4 + decisions 1-4), #3 (missing-skill-star imputation + Montreal market-strip, pts 9-10), and #4 (degenerate peer set for thin/small-sample + extreme-rate rows, pts 11-14) — all DEFERRABLE design questions, nothing changed, nothing pre-decided. Refetch done+verified; exploratory index runs already shown to owner (Ohgren dropped out of surprising list post-fix; top-100 reviewed). Still NO production `compute_oaq` run (gated: Phase-1 hygiene + Gate-4, per airtight plan §E).

OWNER (3 items): (a) eyeball `marchand_index/raw/reddit_identity_pairs.md` (A21 acceptance); (b) YouTube API key -> U1 dry-run + Gate-4; (c) **hand-collect 32-team IG + X follower counts — ASK FOR THESE ON "CONTINUE"**, see OWNER DATA REQUEST section above.

CARRY-FORWARD: 240 tests; pool 771; window [2025-04-18, 2026-04-17]; A12 weights unchanged; impl seed 20260526 / spec seed 20260522 (never harmonize); cache/reddit_corpus/ GITIGNORED LOCAL SOURCE OF RECORD; english_top1000.txt pinned (never edit). Next free amendment number: A45. `fetch_nhl_api.py` patched (extract_skill trade-aggregation) + `raw/nhl_skill.csv` regenerated — COMMITTED in `a216941`, working tree clean.
