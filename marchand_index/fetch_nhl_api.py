...
from _common import (CONTACT_UA, RAW_DIR, WINDOW_SEASONS,  # noqa: E402
                     atomic_write_csv, load_players, session)

API = "https://api-web.nhle.com/v1"
CURRENT_SEASON = "20252026"  # pre-reg A-series locks this as the headline season

# A51: the landing endpoint returns EVERY season in one response, so the
# three-season panel costs no extra requests. One row per (player, season) is
# emitted with a `season` column; CURRENT_SEASON rows reproduce the previous
# single-season file exactly.
#
# DOWNSTREAM: compute_oaq.py still assumes one row per player in nhl_skill.csv.
# It must filter `season == CURRENT_SEASON` before its next run — see A51.
PANEL_SEASONS = sorted(WINDOW_SEASONS)


def season_age(born: dt.date, season: str) -> int:
    """Age on Feb 1 inside the given season (standard hockey age reference).
    season is 'YYYYYYYY'; Feb 1 falls in the second calendar year."""
    ref = dt.date(int(season[4:]), 2, 1)
...
def extract_skill(land: dict | None, season: str = CURRENT_SEASON) -> dict:
    """Age, position, and one season's regular-season PPG + TOI/G."""
    ...
    if bd:
        try:
            out["age"] = season_age(dt.date.fromisoformat(bd), season)
        except Exception:
            pass
...
def main() -> None:
    ...
    for i, p in enumerate(players, 1):
        ...
        land = landing(s, pid) if pid.isdigit() else None
        if not pid.isdigit():
            n_missing += 1
        for season in PANEL_SEASONS:
            info = extract_skill(land, season)
            rows.append({
        ...
        cur = extract_skill(land, CURRENT_SEASON)
        flag = "" if pid.isdigit() else "  <-- NO NHL ID (NULL skill)"
        print(f"[{i}/{len(players)}] {p['full_name']:<24} pid={pid:<9} "
              f"{CURRENT_SEASON} ppg={cur['ppg']} gp={cur['games_played']}{flag}")
        if pid.isdigit():
            time.sleep(0.4)

    atomic_write_csv(RAW_DIR / "nhl_skill.csv", rows, [
        "player_id", "full_name", "nhl_player_id", "team_code", "season",
        "position", "age", "ppg", "toi_per_game", "games_played", "fetch_date",
    ])
    n_ppg = sum(1 for r in rows if r["ppg"] != "")
    per_season = {s: sum(1 for r in rows if r["season"] == s and r["ppg"] != "")
                  for s in PANEL_SEASONS}
    print(f"\nWrote nhl_skill.csv: {len(rows)} rows "
          f"({len(players)} players x {len(PANEL_SEASONS)} seasons; "
          f"{n_ppg} with PPG, {n_missing} missing NHL id)")
    print("  players with PPG per season: "
          + ", ".join(f"{s}={n}" for s, n in per_season.items()))
