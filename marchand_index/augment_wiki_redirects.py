...
day, zero-fill to the full A11/A14 window (WINDOW_DAYS, 921 as of A51),
and rewrite the wiki CSVs
...
# A51: window sourced from _common (start moved back to the 2023-24 opener;
# inclusive end day unchanged). All 365-literals below are now derived.
from _common import (WINDOW_DAYS, WINDOW_END, WINDOW_START,  # noqa: E402,F401
                     WINDOW_START_DATE)
SLEEP_PV = 0.2
SLEEP_MW = 0.15
...
def window_day_index() -> dict[str, int]:
    """'YYYYMMDD00' timestamp -> 0-based index inside the fixed window."""
    out = {}
    for i in range(WINDOW_DAYS):
        d = WINDOW_START_DATE + dt.timedelta(days=i)
...
def zero_fill_365(merged: list[tuple[str, int]]) -> list[int]:
    """Merged per-day sums -> dense WINDOW_DAYS-entry vector (index 0 =
    WINDOW_START_DATE). Days the API omitted are true zero-view days;
    out-of-window stamps drop. (Name kept for call-site stability; the length
    is WINDOW_DAYS, which is 921 as of A51 — not 365.)"""
    vec = [0] * WINDOW_DAYS
    for ts, v in merged:
        i = _DAY_INDEX.get(ts)
        if i is not None:
...
def fetch_daily_pairs(s, pv_domain: str, title: str,
    ...
    if (start, end) == (WINDOW_START, WINDOW_END):
        # A51: the window is now 921 days, so split into ~180-day chunks rather
        # than the two fixed halves. The all-or-nothing rule is UNCHANGED —
        # every chunk must 200 or the whole request fails, so a partial series
        # can never overwrite a good stored value.
        chunks = []
        step = 180
        for off in range(0, WINDOW_DAYS, step):
            c0 = WINDOW_START_DATE + dt.timedelta(days=off)
            c1 = min(WINDOW_START_DATE + dt.timedelta(days=off + step - 1),
                     WINDOW_START_DATE + dt.timedelta(days=WINDOW_DAYS - 1))
            part = fetch_daily_pairs(s, pv_domain, title,
                                     c0.strftime("%Y%m%d"), c1.strftime("%Y%m%d"))
            if part is None:
                return None
            chunks.append(part)
        return merge_daily_by_date(chunks)
    return None
...
def _intl_one_player(s, r: dict, eds: list[str], qid: str):
    ...
    for ed in eds:
        ...
        daily_entries.append({
            "player_id": r["player_id"], "edition": ed, "n_days": WINDOW_DAYS,
