...
START_YEAR = "2025"  # headline season (pre-reg A13); 2025-26 regular season

# A51: three-season panel. MoneyPuck publishes one season-summary file per
# start year, so each season is an independent cached download. Rows carry a
# `season` column; season == '20252026' reproduces the previous single-season
# file exactly.
#
# DOWNSTREAM: compute_oaq.py still assumes one row per player in nhl_onice.csv.
# It must filter `season == '20252026'` before its next run — see A51.
PANEL_YEARS = ["2023", "2024", "2025"]


def mp_url(year: str) -> str:
    return ("https://moneypuck.com/moneypuck/playerData/seasonSummary/"
            f"{year}/regular/skaters.csv")


def cache_csv(year: str):
    return RAW_DIR / f"moneypuck_skaters_{year}.csv"


def season_of(year: str) -> str:
...
CACHE_CSV = cache_csv(START_YEAR)
...
def load_raw(s, year: str = START_YEAR) -> pd.DataFrame:
    """Read one season's cached MoneyPuck CSV, downloading once if absent."""
    dest = cache_csv(year)
    if not dest.exists():
        r = s.get(mp_url(year), headers={"User-Agent": CONTACT_UA}, timeout=60)
        r.raise_for_status()
...
def main() -> None:
    ...
    for year in PANEL_YEARS:
        ...
        raw = load_raw(s, year)
        # MoneyPuck `icetime` is SECONDS; the pre-registered
        # ONICE_MIN_ICETIME_5V5 floor (150) is MINUTES. Convert at ingest so
        # mp_icetime_5v5 is stored in minutes and the floor compares like
        # units. (Rate features are unaffected: icetime-weighted means are
        # scale-invariant.)
        ...
        print(f"[{season}] 5v5 rows: {len(df5v5)}; unique playerIds: "
              f"{len(report)}; playerIds with >=2 5v5 rows "
              f"(in-season trades): {n_traded}")

        agg = aggregate_traded(df5v5)
        floored = [apply_thin_floor(r) for r in agg.to_dict("records")]
        rows = join_pool(players, pd.DataFrame(floored), fetch_date)
        for r in rows:
            r["season"] = season
        counts: dict[str, int] = {}
        for r in rows:
            counts[r["onice_status"]] = counts.get(r["onice_status"], 0) + 1
        print(f"[{season}] joined {len(rows)} pool rows "
              f"(ok={counts.get('ok', 0)}, thin={counts.get('thin', 0)}, "
              f"missing={counts.get('missing', 0)})")
        all_rows.extend(rows)

    atomic_write_csv(OUT_CSV, all_rows, OUT_FIELDS)
    print(f"\nWrote {OUT_CSV}: {len(all_rows)} rows "
          f"({len(players)} players x {len(PANEL_YEARS)} seasons)")
