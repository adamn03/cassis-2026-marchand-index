...
BOOT_SEED = 20260526
N_BOOT = 1000
# A51: the pool grew 774 -> 973, so these were silently reporting the top 0.8%
# and top 7.9% while still labelled 1% and 10%. Derived from the live pool now.
_POOL_N = sum(1 for _ in open(Path(__file__).resolve().parent.parent / "players.csv",
                              encoding="utf-8")) - 1
TOP_K_1PCT = -(-_POOL_N // 100)     # ceil(1% of pool)
TOP_K_10PCT = -(-_POOL_N // 10)     # ceil(10% of pool)
