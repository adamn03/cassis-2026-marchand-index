...
def main() -> None:
    ...
    # MERGE MODE (A51). Existing rows are PRESERVED verbatim and never re-fetched.
    #
    # This file accumulates data this script cannot itself produce: the x_* columns
    # and the google-search-verified IG handles were collected by a separate,
    # partly manual pass (ig_source="google_search_verified:...", 387 rows with real
    # x_followers). A full rebuild writes x_*="null"/"null_uncollected" for every
    # row and would silently destroy all of it. So we only fetch player_ids absent
    # from the existing file — i.e. the pool members added by A51.
    existing = {r["player_id"]: r for r in read_rows(OUT)} if OUT.exists() else {}
    todo = [p for p in attention if p["player_id"] not in existing]
    print(f"player_social merge: {len(existing)} existing rows preserved, "
          f"{len(todo)} new players to fetch")
    if not todo:
        print("nothing to do")
        return

    qids = sorted({qid_by_id.get(p["player_id"], "") for p in todo
                   if qid_by_id.get(p["player_id"], "")})
    handles = wikidata_instagram_handles(qids)
    today = dt.date.today().isoformat()
    rows: list[dict[str, str | int]] = []

    for n, player in enumerate(todo, start=1):
        ...
        if n % 25 == 0:
            print(f"processed {n}/{len(todo)}")

    # Union: preserved rows first, in their original order, then the new ones.
    # Field order comes from the EXISTING file so the schema never silently
    # changes under downstream readers.
    fields = list(next(iter(existing.values()))) if existing else list(rows[0])
    out_rows = list(existing.values()) + [
        {k: r.get(k, "null") for k in fields} for r in rows
    ]
    tmp = OUT.with_suffix(OUT.suffix + ".tmp")          # atomic per vault convention
    with tmp.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(out_rows)
    tmp.replace(OUT)
    n_ok = sum(row["ig_followers"] not in ("null", "") for row in rows)
    n_handles = sum(row["ig_handle"] not in ("null", "") for row in rows)
    print(f"Wrote {OUT} ({len(out_rows)} rows total = {len(existing)} preserved "
          f"+ {len(rows)} new; of the new: {n_handles} verified handles, "
          f"{n_ok} IG counts)")
