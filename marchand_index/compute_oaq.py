...
# A51 downstream: the headline is a single-season quantity (the A11/A14
# attention window is one fixed year), so multi-season panel inputs are
# collapsed to this season on read. Sources spell it differently.
CURRENT_SEASON_SPELLINGS = frozenset({"20252026", "2025-26"})
K_PEERS = 10
...
# A56 (2026-08-31): lambda set to 0 -- the market correction is REMOVED.
# Estimated on 170 club changes, the pass-through is +0.043 [-0.021, +0.108]
# (log scale, weighted across sources) and -0.008 in composite units. Two
# scales, both ~0, neither ~0.5. The components disagree in SIGN:
# en-Wikipedia +0.147 [+0.082, +0.211] but Reddit mentions -0.082
# [-0.161, -0.007]. A market effect is almost certainly real; these three
# sources cannot measure it well enough to subtract, so nothing is
# subtracted. OAQ_portable is consequently identical to OAQ_observed.
LAMBDA_BIGMARKET = 0.0
...
def _current_season_only(df: pd.DataFrame, path: Path) -> pd.DataFrame:
    """A51 downstream fix: collapse a multi-season panel to the headline season.

    A51/A52 turned `nhl_skill.csv`, `nhl_onice.csv`, and `cap_hits.csv` into
    three-season panels (one row per player-season) because the landing
    endpoint returns every season for free. `load_inputs` merges them on
    `player_id` alone, so an unfiltered panel fans the frame out 3x per file --
    measured 26,271 rows against an expected 771. A51 records the obligation:
    "It must filter `season == CURRENT_SEASON` before its next run."

    The headline is a ONE-YEAR quantity by construction: the A11/A14 attention
    window is a fixed trailing 365 days ending 2026-04-17, and OAQ matches that
    attention against same-season production. Trailing seasons belong to the
    A52 event-study panel, never to this frame.

    Season is spelled differently per source ('20252026' vs '2025-26'), so both
    accepted spellings are tried and a file with no `season` column passes
    through untouched. A file that has the column but matches neither spelling
    is a hard error: silently returning zero rows would surface downstream as
    NULL attention rather than as a failure.
    """
    if "season" not in df.columns:
        return df
    s = df["season"].astype(str).str.strip()
    keep = s.isin(CURRENT_SEASON_SPELLINGS)
    if not keep.any():
        raise ValueError(
            f"{path.name}: `season` column present but none of "
            f"{sorted(CURRENT_SEASON_SPELLINGS)} found (saw {sorted(set(s))[:5]})")
    return df[keep].drop(columns=["season"]).reset_index(drop=True)
...
def load_inputs() -> pd.DataFrame:
    ...
    Multi-season panels are collapsed to the headline season on read (A51);
    see `_current_season_only`.
    """
    players = pd.read_csv(PILOT_DIR / "players.csv", dtype={"player_id": int})
    skill = _current_season_only(
        pd.read_csv(RAW_DIR / "nhl_skill.csv", dtype={"player_id": int}),
        RAW_DIR / "nhl_skill.csv")
    onice = _current_season_only(
        pd.read_csv(RAW_DIR / "nhl_onice.csv", dtype={"player_id": int}),
        RAW_DIR / "nhl_onice.csv")
    ...
    wiki_intl = pd.read_csv(RAW_DIR / "wiki_intl_pageviews.csv",
                            dtype={"player_id": int})
    caps = _current_season_only(
        pd.read_csv(RAW_DIR / "cap_hits.csv", dtype={"player_id": int}),
        RAW_DIR / "cap_hits.csv")
...
def stabilize(v: np.ndarray) -> np.ndarray:
    """A57: variance-stabilize a §4 component before standardizing it.

    Attention counts are extremely right-skewed (en-Wikipedia skew 6.9, median
    90,616 against a maximum of 4.2M). A z-score recentres a distribution but
    does not reshape it, so z-scoring raw counts left the composite at skew 6.10
    with the bottom 90% of players inside 11.5% of its range -- almost no
    resolution for nine players in ten, and a peer mean that any one outlier in
    a ten-player set could drag.

    `log1p` was selected on minimum absolute skew of the resulting composite
    (0.30, against 2.38 for sqrt and 0.43 for a Yeo-Johnson fit) from a
    candidate set fixed in A57. It carries no fitted parameter, so there is
    nothing to re-estimate inside the bootstrap and nothing that could be tuned.

    NaN propagates: a missing component must stay missing so the §4 sentinel
    renormalization still fires for it. Counts are clipped at zero first -- the
    components are all non-negative, and this only guards against a malformed
    input reaching `log1p` of a negative number.
    """
...
def compute_engagement_raw(df: pd.DataFrame):
    ...
    comp_z = {c: zscore_array(stabilize(df[c].to_numpy(dtype=float)))
              for c in COMPONENTS}
...
# A54 (2026-08-31) — the market proxy is a weighted composite, not an
# equal-weight z-mean. Equal thirds gave a third of the instrument to metro
# population, the least informative component available (club-level rho +0.055,
# player-level -0.011). Reweighting toward the components that carry signal
# lifts the club-level proxy from rho +0.261 (p=.15) to +0.382 (p=.031).
#
# The social component broadens from Reddit alone to Reddit + club Instagram +
# club X, all three log1p'd before standardizing because follower counts are
# heavy-tailed across 32 clubs and a raw z lets one market dominate. Metro
# population is log1p'd for the same reason. Attendance is already a percentage.
#
# This does NOT rescue the correction: the reweighted proxy is still null at the
# PLAYER level (+0.061, p=.11), which is where lambda is applied. That is
# disclosed in A54 and published as a finding, not buried. lambda stays 0.5.
A54_SOCIAL_COLS = ["team_sub_subscribers", "team_ig_followers",
                   "team_x_followers"]
A54_WEIGHTS = {"social": 0.40, "attendance": 0.40, "population": 0.20}
...
def _attach_team_social(mp: pd.DataFrame) -> pd.DataFrame:
    """A54: join club Instagram / X follower counts onto the market proxy.

    Returns a COPY so the caller's frame — and the audit lenses computed from
    it — are untouched. Absent or incomplete `team_social.csv` leaves the
    columns off entirely, and §7 graceful degradation then drops them from the
    social component rather than silently scoring those clubs as zero.
    """
    path = RAW_DIR / "team_social.csv"
    if not path.exists():
        return mp
    ts = pd.read_csv(path)
    cols = {"ig_followers": "team_ig_followers", "x_followers": "team_x_followers"}
    keep = ["team_code"] + [c for c in cols if c in ts.columns]
    if len(keep) == 1:
        return mp
    return mp.merge(ts[keep].rename(columns=cols), on="team_code", how="left")


def _market_z_a54(mp: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    """A54 weighted market composite: 0.40 social + 0.40 attendance + 0.20 pop.

    Heavy-tailed counts (followers, population) are log1p'd before
    standardizing; attendance is already a share. §7 graceful degradation is
    preserved — a component absent for any of the 32 clubs is dropped, and the
    social block is rebuilt from whichever of its three parts survive.
    """
    def _z_log(col: str) -> np.ndarray | None:
        if col not in mp.columns:
            return None
        v = pd.to_numeric(mp[col], errors="coerce").to_numpy(dtype=float)
        if np.isfinite(v).sum() < len(mp):
            return None
        return zscore_array(np.log1p(v))

    used: list[str] = []
    social_parts = []
    for c in A54_SOCIAL_COLS:
        z = _z_log(c)
        if z is not None:
            social_parts.append(z)
            used.append(c)

    blocks, weights = [], []
    if social_parts:
        blocks.append(zscore_array(np.sum(social_parts, axis=0)))
        weights.append(A54_WEIGHTS["social"])

    att = pd.to_numeric(mp.get("attendance_pct_capacity"),
                        errors="coerce").to_numpy(dtype=float) \
        if "attendance_pct_capacity" in mp.columns else np.array([])
    if att.size == len(mp) and np.isfinite(att).all():
        blocks.append(zscore_array(att))
        weights.append(A54_WEIGHTS["attendance"])
        used.append("attendance_pct_capacity")

    pop = _z_log("metro_population")
    if pop is not None:
        blocks.append(pop)
        weights.append(A54_WEIGHTS["population"])
        used.append("metro_population")

    if not blocks:
        raise RuntimeError("A54: no market components present for all 32 teams.")
    w = np.asarray(weights, dtype=float)
    w = w / w.sum()                     # renormalize if a block degraded away
    composite = np.tensordot(w, np.vstack(blocks), axes=1)
    return zscore_array(composite), used
...
def compute_market_z(df: pd.DataFrame, mp: pd.DataFrame | None = None,
    ...
    mp = _attach_team_social(mp)
    z_primary, used = _market_z_a54(mp)
...
def bootstrap_player_cis(
    ...
    for d in range(n_draws):
        ...
        # A57: resampled draws are stabilized exactly as the point estimate is,
        # or the intervals would be computed on a different scale than the value
        # they surround.
        comp_z = {
            "wiki_12mo": zscore_array(stabilize(wiki_draw)),
            "wiki_intl_12mo": zscore_array(stabilize(intl_draw)),
            "reddit_mentions_12mo": zscore_array(stabilize(rmen_draw)),
            "reddit_upvotes_12mo": zscore_array(stabilize(rup_draw)),
            "trends_12mo": zscore_array(stabilize(trends_fixed)),
        }
        er = engagement_from_components(comp_z)
