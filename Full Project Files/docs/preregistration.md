...
**A10 (2026-06-17) — Scope note: the CASSIS poster run.** The abstract was accepted for a poster (session 2026-09-12). The artifact built for the poster is the **pilot2 method** (`pilot2/preregistration.md`: §4 composite + weights, §6 K=10 Mahalanobis peer matching, §7/A5 λ=0.5 market correction, §8/A8 hybrid headline, §9/A6 V1/V2/V3 gates, §10 bootstrap, A9 OAuth) run over a **whole-league pool** — every rostered skater (F+D, goalies excluded) on the 32 teams' end-of-2025-26 rosters who has played ≥1 NHL regular-season game, **locked at 774 skaters (497 F / 277 D), snapshot 2026-06-17**. This pool redefinition is logged in full as **A10 in `pilot2/preregistration.md §14`**, with a `pool_gp_audit.csv` keep/drop trail. The 774-skater pool covers most of this full-build document's intended ~700-NHLer population, but the poster run does **not** execute this document's full-build-only apparatus — Gate 4 (YouTube held-out residual test, §5–§8), H1–H4 (§3), and the LLM theme-classifier audit (§4 gate 5) remain future work and are not claimed on the poster. This document's hypotheses, gates, and bootstrap seed (`20260522`) are unchanged; this note only records which scope was actually executed for the September poster.

**A11 (2026-06-19) — Reddit attention window fixed to the regular season (poster run).** The Reddit engagement window changes from a trailing 365 days anchored to the fetch moment to a **fixed** trailing-365-day window **ending the last day of the 2025-26 regular season (2026-04-17 inclusive**, api-web.nhle.com `standingsEnd` for season `20252026`). Reason: the 774-set fetch runs during the 2026 playoffs, and OAQ peer-matches attention against regular-season production, so a fetch-anchored window would bake the 2026 playoff run into "attention" and bias the metric toward players whose teams *made the playoffs* — a confound a stats-literate reviewer would attack. Fixing the endpoint to the regular-season boundary aligns the attention window with the production window, gives every player an identical window independent of run-time, and keeps the 365-day length so the `*_12mo` magnitude stays pilot-comparable. The full amendment — mechanism (Reddit `t=year → all`, newest-first skip/stop paging), the honest residual prior-season (2024-25) playoff overlap at the window's far edge disclosed as a limitation, and the abandonment of the stale 2026-05-27 cache reuse — is logged canonically as **A11 in `pilot2/preregistration.md §14`**. This document's gates, hypotheses, and seed are unchanged.

**G4-A1 (2026-07-18) — §7.2 relevance criterion tightened: LAST-name title match; shared-surname disambiguation; identical-name exclusion. Logged BEFORE any Gate-4 YouTube fetch. Cross-ref: `docs/airtight_execution_plan.md` §B Gate-4 amendments (source E6).**
