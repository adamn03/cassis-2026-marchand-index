...
- **A10 (2026-06-17)** — **Player set expanded from the 160-skater Tier-1 slice to the whole league.** With the abstract accepted for a poster, the owner expanded the pool from each team's top-deployed forwards + top defense pair (A7, 160) to *every rostered skater* (forwards + defensemen, goalies excluded) on all 32 teams' end-of-2025-26 rosters. The roster was captured once from the NHL public roster endpoint on 2026-06-17 and locked (it would be overwritten by July 1 free agency). Because a mid-June roster pull also lists signed prospects who have never played in the NHL — players with no NHL stats to compare against — the pool keeps only skaters who **have played at least one NHL regular-season game in their career**. This is a clean cutoff with no arbitrary number: it keeps a star who missed the whole season injured (e.g. Aleksander Barkov, who has no 2025-26 games but eight prior full seasons) and drops only the 14 never-played prospects. **Final pool: 774 skaters (497 forwards, 277 defensemen)**, up from 160. The method itself does not change — same five attention signals and weights, same K=10 peer matching, same market correction, same headline denominator, same validation gates — only the number of players grows, which makes the peer groups larger and the matching steadier. A new **`small_sample`** label marks any player with fewer than 20 games this season (or none): these players stay in every calculation, but the label is a plain warning not to quote a headline number off a tiny-sample call-up. Because changing who is in the set re-runs the validation gates, the new gate results (jersey, All-Star, team-level) are reported in whichever direction they come out, against the same pre-set thresholds. Logged before any data was fetched or computed on the new set.
